...
class InstallBaseEnvConsoleView(base_view.BaseConsoleView):
    ...
    def show(self):
        ...
        self.need_run_cmd_list.append(f'pve_cli ssh ssh-passwordless-to-host {self.selected_hostname}')
        self.need_run_cmd_list.append(f'pve_cli host install-base-env --host {self.selected_hostname}')
        if self.selected_hostname not in CONF.base_env.installed_nodes:
            ...
            self.need_run_cmd_list.append(f'pve_cli ssh ssh-run-on-remote {self.selected_hostname} "pve_cli host set-ntp-server {other_node_ntp_server_ip}"')
        self.start_alarm()
