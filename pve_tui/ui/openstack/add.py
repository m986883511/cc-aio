...
class AddComputeConsoleView(base_view.BaseConsoleView):
    ...
    def show(self):
        ...
        self.need_run_cmd_list.append(f'pve_cli network check-kolla-interface-exist {self.selected_hostname}')
        self.need_run_cmd_list.append(f'pve_cli kolla add-compute-node {self.selected_hostname}')
        if os.path.exists(self.ceph_config_dir):
            self.need_run_cmd_list.append(f'pve_cli ssh rsync-dir-to-remote-host {self.selected_hostname} {self.ceph_config_dir}')
        self.start_alarm()
