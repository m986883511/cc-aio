...
from pve_tui.config import CONF, PVE_TUI_CONF_PATH
...
def install_new_master_astute_package(auto_install_flag=False):
    ...
    if name == 'yes':
        ...
        flag = execute.execute_command_in_popen(f'systemctl restart pve_rpc')
        execute.completed(flag, 'restart pve_rpc')
    else:
...
def get_other_nodes_ntp_server_config():
    LOG.info('reload pve_tui config')
