...
function change_pool_default_size(){
    ...
    local hostadmin_ceph_conf_path=$(python -c "from hostadmin.files import FilesDir; print(FilesDir.Ceph.conf)")
...
function scp_etc_conf_to_master_node(){
    ...
    if [ -v SSH_CONNECTION ]; then
        if [ "$calling_script" = 'sshd' ]; then
            ...
            hostcli ssh scp-dir-to-remote-host $REPO_SERVER_IP /etc/ceph /etc/
