...
ACD_CONFIG_PATH="/etc/cs/pve_tui.conf"
CURRENT_NODE_222_IP=
...
function wait_current_node_install_ceph_completed(){
    ...
    while true; do
        # 执行需要重试的指令
        pve_cli ceph ceph-orch-ps-current-node
        if [ $? -eq 0 ]; then
...
function restart_ceph_about_container(){
    ...
    for node in $control_nodes; do
        echo "restart $node"
        pve_cli ssh ssh-run-on-remote $node "pve_cli ceph restart-ceph-about-container"
