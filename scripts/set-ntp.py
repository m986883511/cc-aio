...
from pve_tui.utils import get_other_nodes_ntp_server_config
...
class SetNtpTask:
    ...
    def check_network_connection(self):
        for node in self.nodes_but_not_openstack_node:
            flag, content = execute.execute_command(f'pve_cli network check-network-connection {node}')
            execute.completed(flag, f'check {node} network connection', content)
            flag, content = execute.execute_command(f'pve_cli ssh check-ssh-passwordless {node}')
    ...
    def set_ntp(self):
        for node in self.nodes_but_not_openstack_node:
            flag, content = execute.execute_command(f'pve_cli ssh ssh-run-on-remote {node} "pve_cli host set-ntp-server {self.other_node_ntp_server_ip}"')
...
if __name__ == "__main__":
    ...
    func.set_simple_log('/var/log/astute/set_ntp.log')
